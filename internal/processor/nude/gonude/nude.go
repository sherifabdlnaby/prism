...
// THIS FILE IS A MODIFIED FILE FROM github.com/koyachi/go-nude
...
func CensorSkinPixels(src image.Image, regions Regions, color color.RGBA) image.Image {
	dstImg := image.NewRGBA(src.Bounds())
	draw.Draw(dstImg, src.Bounds(), src, image.ZP, draw.Src)
	for i := 0; i < len(regions); i++ {
		for _, pixel := range regions[i] {
			dstImg.Set(pixel.X, pixel.Y, color)
		}
	}
	return dstImg
}
