...
var (
	...
	errNoAck            = err{Code: http.StatusBadRequest, Message: "request was dropped on purpose"}
...
func respondError(req *http.Request, w http.ResponseWriter, error err) {
...
func respondMessage(req *http.Request, w http.ResponseWriter, statusCode int, JSON interface{}) {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(statusCode)
	jsonBuf, _ := json.Marshal(JSON)
	_, _ = w.Write(jsonBuf)
}
